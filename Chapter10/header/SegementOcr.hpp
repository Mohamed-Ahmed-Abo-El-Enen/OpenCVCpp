...
vector<RotatedRect> FindTextAreas_OCR(Mat input) {
	//Dilate the image
...
Mat DeskewAndCrop_OCR(Mat input, const RotatedRect& box)
{
...
const char* IdentifyText_OCR(Mat input, const char* language = "eng")
{	
...
int SegementOcr(int argc, char* argv[]) 
{
	//Loads the ticket image and binarize it
	Mat ticket = Binarize_OCR(imread("data/raw/text.jpg"));	
	auto regions = FindTextAreas_OCR(ticket);
	...
	for (const auto& region : regions) {
		//Crop 
		auto cropped = DeskewAndCrop_OCR(ticket, region);
		auto text = IdentifyText_OCR(cropped, "eng");
